...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        xyz_flipped = xyz.transpose(1, 2).contiguous()
        if inds is None:
            inds = pointnet2_utils.furthest_point_sample(xyz, self.npoint)      # fps2048个索引
        else:
            assert(inds.shape[1] == self.npoint)
        new_xyz = pointnet2_utils.gather_operation(
            xyz_flipped, inds.int()
        ).transpose(1, 2).contiguous() if self.npoint is not None else None     # 采样的点B 2048 3

        if not self.ret_unique_cnt:
            grouped_features, grouped_xyz = self.grouper(
                xyz, new_xyz, features
            )  # (B, C, npoint, nsample)   grouped_features == grouped_xyz 而且是相对坐标
        else:
        ...
        new_features = self.mlp_module(
            grouped_features
        )  # (B, mlp[-1], npoint, nsample)      torch.Size([8, 3, 2048, 64])标准化后的坐标->torch.Size([8, 256, 2048, 64])     从3-256
        if self.pooling == 'max':
            new_features = F.max_pool2d(
                new_features, kernel_size=[1, new_features.size(3)]
            )  # (B, mlp[-1], npoint, 1)        64近邻合并成一个
        ...
        new_features = new_features.squeeze(-1)  # (B, mlp[-1], npoint)     torch.Size([8, 256, 2048])
...
class PointnetSAModuleVotesUPMAE(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def forward(self, xyz: torch.Tensor,
                features: torch.Tensor = None,
                inds: torch.Tensor = None, bool_masked_pos: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        r"""
        Parameters
        ----------
        xyz : torch.Tensor
        ...
        B, C, _ = new_features.shape
        new_xyz = new_xyz[~bool_masked_pos].reshape(B, -1, 3)
        new_features = new_features.permute(0,2,1)[~bool_masked_pos].reshape(B, -1, C).permute(0,2,1)
        inds = inds[~bool_masked_pos].reshape(B, -1)
