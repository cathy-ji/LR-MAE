...
from utils.io import save_checkpoint, resume_if_possible, load_model_from_ckpt
...
def make_args_parser():
    ...
    parser.add_argument(
        "--model_name",
        default="up_mae_3detr_kps",
        type=str,
        help="Name of the model",
        choices=["3detr","umae_3detr","up_mae_3detr","umae","up_mae_3detr_kps"],
    )
    ### Encoder
    parser.add_argument(
    ...
    parser.add_argument('--loss_query_kps_weight', default=0.8, type=float)

    ##### Dataset #####
    parser.add_argument(
        "--dataset_name", default="scannet", type=str, choices=["scannet", "sunrgbd", "selfscannet"]
    )
    parser.add_argument(
        "--dataset_root_dir",
        type=str,
    ...
    parser.add_argument("--dist_url", default="tcp://localhost:12355", type=str)
        ...
        if epoch % args.eval_every_epoch == 0 or epoch == (args.max_epoch - 1):
            ...
            if is_primary():
                ...
                with open(final_eval, "a") as fh:
                    fh.write("==" * 10)
                    fh.write("\n")
                    fh.write(f"Evaluate Epoch [{epoch}/{args.max_epoch}]; Metrics {metric_str}")
                    fh.write("\n")
            ...
            if is_primary() and (
                ...
                print(
                    f"Epoch [{epoch}/{args.max_epoch}] saved current good val checkpoint at {filename}; ap25 {ap25}"
                )  
...
def test_model(args, model, model_no_ddp, criterion, dataset_config, dataloaders):
    ...
    for batch_idx, batch_data_label in enumerate(dataloaders["test"]):
        ...
        mesh_vertices = batch_data_label["mesh_vertices"][0]
        instance_bboxes = batch_data_label["instance_bboxes"][0]
        prediction = torch.cat([outputs["center_unnormalized"][0],outputs["size_unnormalized"][0],outputs["objectness_prob"][0].unsqueeze(-1)],dim=-1)
        np.save("./vis/scan_prediction_"+str(batch_idx)+".npy",prediction.cpu().detach().numpy())
        np.save("./vis/scan_pc_"+str(batch_idx)+".npy",mesh_vertices.cpu().detach().numpy())
        np.save("./vis/scan_gt_"+str(batch_idx)+".npy",instance_bboxes.cpu().detach().numpy())
        if batch_idx>30:
            sys.exit(-1)
...
def main(local_rank, args):
    ...
    if args.pretrain_ckpt:
        load_model_from_ckpt(args.pretrain_ckpt, model_no_ddp)
