...
class PointcloudScaleAndNormal(object):
    def __init__(self, scale_low=2. / 3., scale_high=3. / 2.):
        self.scale_low = scale_low
        self.scale_high = scale_high
    ...
    def __call__(self, pc):
        ...
        for i in range(bsize):
            ...
            temp = torch.mul(pc[i, :, 0:3], torch.from_numpy(xyz1).float().cuda())

            centroid = torch.mean(temp, dim=0)
            temp = temp - centroid
            m = torch.max(torch.sqrt(torch.sum(temp ** 2, dim=1)))
            temp = temp / m
