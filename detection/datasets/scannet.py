...
DATASET_ROOT_DIR = "/votenet/scannet/scannet_train_detection_data"  ## Replace with path to dataset
DATASET_METADATA_DIR = "/votenet/scannet/meta_data" ## Replace with path to dataset
...
class SelfScannetDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        self.dataset_config = dataset_config
        assert split_set in ["train", "val", "all"]
        if root_dir is None:
            root_dir = DATASET_ROOT_DIR
    ...
    def calculate_bounding_boxes(self, point_cloud, n=10, num_neighbors=2048):
        
        random_point_indices = np.random.choice(point_cloud.shape[0], n, replace=False)
        center_points = torch.tensor(point_cloud[random_point_indices], dtype=torch.float32)
        bounding_boxes = []
        ...
        group_indices = self.knn_point(num_neighbors, point_cloud.unsqueeze(0), center_points.unsqueeze(0)).squeeze()#S nsample
        nearest_points_all = []
        ...
        bounding_boxes = torch.stack(bounding_boxes).numpy()  
        nearest_points_all = torch.stack(nearest_points_all).numpy()
        return bounding_boxes, nearest_points_all, center_points.numpy()
    ...
    def __getitem__(self, idx):
        ...
        instance_bboxes, nearest_point, center_points = self.calculate_bounding_boxes(point_cloud, n=self.num_blocks)
        ...
        ret_dict = {}
        ret_dict["point_clouds"] = point_cloud.astype(np.float32)
        ret_dict["point_clouds_center"] = center_points.astype(np.float32)
        ret_dict["point_clouds_nearest"] = nearest_point.astype(np.float32)
        ret_dict["gt_box_corners"] = box_corners.astype(np.float32)
        ret_dict["gt_box_centers"] = box_centers.astype(np.float32)
        ret_dict["gt_box_centers_normalized"] = box_centers_normalized.astype(
            np.float32
...
class UpmaeScannetDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
    ...
    def __getitem__(self, idx):
        scan_name = self.scan_names[idx]
        mesh_vertices = np.load(os.path.join(self.data_path, scan_name) + "_vert.npy")      # 50000个点每个点xyzrgb
        # instance_bboxes = np.load(os.path.join(self.data_path, scan_name) + "_bbox.npy")    # n个box（n不确定） n*7
        if not self.use_color:
            point_cloud = mesh_vertices[:, 0:3]  # do not use color for now
            pcl_color = mesh_vertices[:, 3:6]
        else:
