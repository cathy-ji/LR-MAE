...
class Pointnet2Backbone(nn.Module):
    ...
    def __init__(self, input_feature_dim=0, width=1, depth=2):
        ...
        self.fp2 = PointnetFPModule(mlp=[256 * width + 256 * width, 256 * width, 256])


    def forward(self, xyz, features,inds=None):
        r"""
            Forward pass of the network

            Parameters
            ----------
            xyz: Variable(torch.cuda.FloatTensor)
                (B, N, 3 ) tensor
                Point cloud to run predicts on
                Each point in the point-cloud MUST
                be formated as (x, y, z)
            features: None or (torch.cuda.FloatTensor)
                (B,N,C)
        ...
        xyz, features, fps_inds = self.sa1(xyz, features,inds)
        ...
        return end_points['sa2_xyz'],features,end_points['fp2_inds']
