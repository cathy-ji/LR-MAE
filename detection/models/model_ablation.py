...
from third_party.pointnet2.pointnet2_modules import PointnetSAModuleVotes, PointnetSAModuleVotesUPMAE
...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        self.increase_dim = nn.Conv1d(decoder_dim, 3 * 64, 1)
    ...
    def run_encoder(self, point_clouds, inds, bool_masked_pos):
        xyz, features = self._break_up_pc(point_clouds)  
        pre_enc_xyz, pre_enc_features, pre_enc_inds = self.pre_encoder(xyz, features, inds, bool_masked_pos)  # embedding npoint=2048-256
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features):
        ...
        center_offset = (  # torch.Size([64, 256, 3]) 
                self.mlp_heads["center_head"](box_features).sigmoid().transpose(1, 2) - 0.5
        )
        size_normalized = (  # torch.Size([64, 256, 3]) 
    ...
    def forward(self, inputs, encoder_only=False):
        ...
        neighb = inputs["point_clouds_nearest"]  # (B 256 64 3)
        query_xyz = inputs["point_clouds_center"]  # （B 256 3）

        enc_xyz, enc_features, enc_inds = self.run_encoder(
            point_clouds, inds, bool_masked_pos)  # torch.Size([B, 1792, 3])      torch.Size([1792, B, 256])
        enc_features = self.encoder_to_decoder_projection(
            enc_features.permute(1, 2, 0)
        ).permute(2, 0, 1)  
        enc_pos = self.pos_embedding(enc_xyz, input_range=point_cloud_dims)  # (B 256 1792)
        enc_pos = enc_pos.permute(2, 0, 1)  # torch.Size([1792, 8, 256])

        pos_embed = self.pos_embedding(query_xyz, input_range=point_cloud_dims)  # (B C 256)
        query_embed = self.query_projection(pos_embed)  # 256维-256维 torch.Size([B, C, 256])
        query_embed = query_embed.permute(2, 0, 1)  # torch.Size([256, 8, 256])     
        tgt = self.mask_token.expand_as(query_embed)  # torch.Size([256, 8, 256])    
        box_features = self.decoder(
            tgt, enc_features, query_pos=query_embed, pos=enc_pos
        )[0]  # box_features  torch.Size([8, 256, B, 256]) 
        rec_feature = box_features[-1].permute(1,2,0).contiguous()  # (npoint B C)->(B C npoint )

        B, C, npoint = rec_feature.shape
        origion_points = neighb - query_xyz.unsqueeze(2)  # (B 256 64 3)

        rec_points = self.increase_dim(rec_feature).transpose(1, 2).reshape(B * npoint, -1, 3)  # (Bxnpoint 64 3)
        gt_points = origion_points.reshape(B * npoint, -1, 3)
        loss_rec = self.loss_func(rec_points, gt_points)
        if encoder_only:
...
def build_preencoder(args):
    ...
    preencoder = PointnetSAModuleVotesUPMAE(
        radius=0.2,
        nsample=64,
        npoint=args.preenc_npoints - args.nqueries,
        mlp=mlp_dims,
        normalize_xyz=True,
    )
...
def build_ablation(args, dataset_config):
