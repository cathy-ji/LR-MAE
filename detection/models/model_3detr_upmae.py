...
from models.backbone_encoder import Pointnet2Backbone
...
class FPSModule(nn.Module):
    ...
    def forward(self, xyz, features):
        ...
        new_features = pointnet2_utils.gather_operation(features_flipped, sample_inds).contiguous()
...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        # self.fps_module = FPSModule(num_queries)
    ...
    def get_query_embeddings(self, encoder_xyz, point_cloud_dims):
        query_inds = furthest_point_sample(encoder_xyz, self.num_queries)  # fps256个查询点
        query_inds = query_inds.long()
        query_xyz = [torch.gather(encoder_xyz[..., x], 1, query_inds) for x in range(3)]
        query_xyz = torch.stack(query_xyz)  # 将3个list合并成一个3维Tensor，相当于将x、y、z合并
        query_xyz = query_xyz.permute(1, 2, 0)  # 得到查询点 torch.Size([8, 256, 3])

        # Gater op above can be replaced by the three lines below from the pointnet2 codebase
        # xyz_flipped = encoder_xyz.transpose(1, 2).contiguous()
        # query_xyz = gather_operation(xyz_flipped, query_inds.int())
        # query_xyz = query_xyz.transpose(1, 2)
        pos_embed = self.pos_embedding(query_xyz, input_range=point_cloud_dims)  # 得到查询点的位置编码
        query_embed = self.query_projection(pos_embed)  # 256维-256维 torch.Size([8, 256, 256])
        return query_xyz, query_embed

    def get_query_xyz_embeddings(self, encoder_xyz, feature, point_cloud_dims):
        query_inds = furthest_point_sample(encoder_xyz, self.num_queries)  # fps256个查询点
        query_inds = query_inds.long()
        query_xyz = [torch.gather(encoder_xyz[..., x], 1, query_inds) for x in range(3)]
        query_xyz = torch.stack(query_xyz)  # 将3个list合并成一个3维Tensor，相当于将x、y、z合并
        ...
        query_feature = [torch.gather(feature[..., x], 1, query_inds) for x in range(feature.size(2))]
        query_feature = torch.stack(query_feature)  # 将3个list合并成一个3维Tensor，相当于将x、y、z合并
        query_feature = query_feature.permute(1, 2, 0)  # 得到查询点 torch.Size([8, 256, 3])
        ...
        pos_embed = self.pos_embedding(query_xyz, input_range=point_cloud_dims)  # 得到查询点的位置编码
        query_embed = self.query_projection(pos_embed)  # 256维-256维 torch.Size([8, 256, 256])
        return query_xyz, query_feature, query_embed
    ...
    def run_encoder(self, point_clouds):
        xyz, features = self._break_up_pc(point_clouds)  # 拿xyz
        pre_enc_xyz, pre_enc_features, pre_enc_inds = self.pre_encoder(xyz, features)  # 中心点坐标，embedding
        ...
        enc_xyz, enc_features, enc_inds = self.encoder(  # 返回的是xyz, output, None
            pre_enc_features, xyz=pre_enc_xyz
        )  # Encoder中xyz啥也没干 只是pre_enc_features这个相对坐标的embedding过Transformer
        if enc_inds is None:
            # encoder does not perform any downsampling
            enc_inds = pre_enc_inds  # 将fps的2048个的坐标赋值
        else:
            # use gather here to ensure that it works for both FPS and random sampling
            enc_inds = torch.gather(pre_enc_inds, 1, enc_inds.type(torch.int64))
        return enc_xyz, enc_features, enc_inds  # 中心点torch.Size([8, 2048, 3])、相对局部的embedding torch.Size([2048, 8, 256]), 中心点的索引
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features):
        ...
        box_features = box_features.permute(0, 2, 3,
                                            1)  # torch.Size([8, 8, 256, 256])      num_layers, batch, channel, num_queries
        ...
        for l in range(num_layers):
            ...
            (
                ...
                center_offset[l], query_xyz, point_cloud_dims  # 查询点+第一层的offset        256个新的中心点
            )
            angle_continuous = self.box_processor.compute_predicted_angle(
                angle_logits[l], angle_residual[l]  # 全0
            )
            size_unnormalized = self.box_processor.compute_predicted_size(
                size_normalized[l], point_cloud_dims  # torch.Size([8, 256, 3])
            )
            box_corners = self.box_processor.box_parametrization_to_corners(
                center_unnormalized, size_unnormalized, angle_continuous  # 上面算的 计算
            )
    ...
    def forward(self, inputs, encoder_only=False):
        ...
        enc_xyz, enc_features, enc_inds = self.run_encoder(
            point_clouds)  # torch.Size([4, 2048, 3])      torch.Size([2048, 4, 256])
        enc_features = self.encoder_to_decoder_projection(
            enc_features.permute(1, 2, 0)
        ).permute(2, 0, 1)  # 256-256的非线性层，由3个Conv1d组成      由torch.Size([2048, 8, 256])到torch.Size([2048, 8, 256])
        ...
        query_xyz, tgt, query_embed = self.get_query_xyz_embeddings(enc_xyz, enc_features.transpose(0, 1).contiguous(),  point_cloud_dims)  # 从2048个点中获取查询点
        # query_embed: batch x channel x npoint
        enc_pos = self.pos_embedding(enc_xyz, input_range=point_cloud_dims)  # fps的那2048个点做位置编码

        # decoder expects: npoints x batch x channel
        enc_pos = enc_pos.permute(2, 0, 1)  # torch.Size([2048, 8, 256])    2048个点的
        query_embed = query_embed.permute(2, 0, 1)  # torch.Size([256, 8, 256])     256个查询点的
        # _, tgt, _ = self.fps_module(enc_xyz, enc_features.transpose(0, 1).contiguous())
        tgt = tgt.transpose(0, 1).contiguous()
        box_features = self.decoder(
            tgt, enc_features, query_pos=query_embed, pos=enc_pos
        )[0]  # box_features  torch.Size([8, 256, 8, 256]) 8层decoder的每层结果
...
def build_3detr_upmae(args, dataset_config):
