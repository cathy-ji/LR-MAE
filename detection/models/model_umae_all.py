...
from models.MAE import Point_MAE, Point_MAE_LEM
...
class BoxProcessor(object):
    ...
    def compute_objectness(self, cls_logits):
        assert cls_logits.shape[-1] == self.dataset_config.num_semcls + 1
        cls_prob = torch.nn.functional.softmax(cls_logits, dim=-1)
        objectness_prob = 1 - cls_prob[..., -1]
        return objectness_prob
...
class Model3DETR(nn.Module):
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features):
        ...
        for l in range(num_layers):
            ...
            # with torch.no_grad():
            #     objectness_prob = self.box_processor.compute_objectness(cls_logits[l])
    ...
    def forward(self, inputs, encoder_only=False):
        ...
        neighb = inputs["point_clouds_nearest"]
        center = inputs["point_clouds_center"]

        pts = neighb - center.unsqueeze(2)
        bs, num_block, n, _ = pts.shape
        pts = pts.reshape(bs*num_block,n,_)
        loss_rec, blocks_feat = self.mae(pts)
        ...
        query_xyz, query_embed = self.get_query_embeddings(enc_xyz, point_cloud_dims)  # 从2048个点中获取查询点
        ...
        tgt = torch.zeros_like(query_embed) + blocks_feat # torch.Size([256, 8, 256])     全0
        ...
        return box_predictions, loss_rec
...
def build_umae_all(args, dataset_config):
