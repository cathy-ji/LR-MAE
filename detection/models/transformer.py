    ...
    def forward(self, src,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            output = layer(output, src_mask=mask,
                           src_key_padding_mask=src_key_padding_mask, pos=pos)      # 只有output非空
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer in self.layers:           # 这里有8层TransformerDecoderLayer
        ...
        if self.norm is not None:
            output = self.norm(output)
            if self.return_intermediate:            # 返回中间的值
                intermediate.pop()
...
class MaskedTransformerEncoder(TransformerEncoder):
    ...
    def compute_mask(self, xyz, radius, dist=None):
        with torch.no_grad():
            ...
            mask = dist >= radius       # 距离大于radis为True
        return mask, dist
    ...
    def forward(self, src,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.masking_radius[idx] > 0:
                ...
                nhead = layer.nhead
                mask = mask.unsqueeze(1)
                mask = mask.repeat(1, nhead, 1, 1) # 每个头都有个mask
                mask = mask.view(bsz * nhead, n, n)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory,                  # tgt prompt feature，memory 2048个点的feature
                    tgt_mask: Optional[Tensor] = None,
                    memory_mask: Optional[Tensor] = None,
                    tgt_key_padding_mask: Optional[Tensor] = None,
                    memory_key_padding_mask: Optional[Tensor] = None,
                    ...
                    query_pos: Optional[Tensor] = None,
                    return_attn_weights: Optional [bool] = False):
        tgt2 = self.norm1(tgt)                          # prompt过normal
        q = k = self.with_pos_embed(tgt2, query_pos)    # prompt加上它的pos emb 赋值给q和k
        tgt2 = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]     # v却是加上pos之前的
        tgt = tgt + self.dropout1(tgt2)                                     # 残差
        tgt2 = self.norm2(tgt)
        tgt2, attn = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),        # q是prompt+pos
                                   key=self.with_pos_embed(memory, pos),                    # key是2048个点的feature+pos
                                   value=memory, attn_mask=memory_mask,                     # v是2048个点的feature
                                   key_padding_mask=memory_key_padding_mask)
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt2))))              # FFN层
        tgt = tgt + self.dropout3(tgt2)
        if return_attn_weights:
            return tgt, attn
        return tgt, None
...
class TransformerwithMAEDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=256,
        ...
        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.norm0 = NORM_DICT[norm_fn_name](d_model)
        self.norm1 = NORM_DICT[norm_fn_name](d_model)
        self.norm2 = NORM_DICT[norm_fn_name](d_model)
    ...
    def forward_pre(self, tgt, memory,                  # tgt prompt feature，memory 2048个点的feature
        ...
        x_full = torch.cat([tgt, memory],dim=0)#(2048 B 256)
        pos_full = torch.cat([query_pos,pos])#(2048 B 256)
        x_full = self.norm0(x_full)
        q = k = self.with_pos_embed(x_full,pos_full)
        x_full = self.self_attn(q, k, value=x_full, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]     # v却是加上pos之前的
        tgt = x_full[:256]#(256 B 256)
        memory = x_full[256:]
