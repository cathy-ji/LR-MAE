...
def get_args():
    ...
    parser.add_argument(
        '--config', 
        default = 'cfgs/finetune_scan_objbg_featMix.yaml',
        type = str, 
    ...
    args.experiment_path = os.path.join('./experiments5', Path(args.config).stem, Path(args.config).parent.stem, args.exp_name)
    args.tfboard_path = os.path.join('./experiments5', Path(args.config).stem, Path(args.config).parent.stem,'TFBoard' ,args.exp_name)
