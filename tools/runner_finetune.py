...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        # if epoch==20:
        #     warmup_model = copy.deepcopy(base_model)
        #     print("save the warmup_model")
        # is_feature_mixup_turn = epoch>=20
        if args.distributed:
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if isinstance(data[0],list):
                points = torch.cat(data[0],dim=0).cuda()
                label = torch.cat(data[1],dim=0).cuda()
            else:
                points = data[0].cuda()
                label = data[1].cuda()
            ...
            #     attention = warmup_model(points,is_feature_mixup_turn=False,return_attn=True)
            #     ret = base_model(points,attn=attention,is_feature_mixup_turn=True)
            # else:
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 10):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            if isinstance(data[0],list):
                points_raw = torch.cat(data[0],dim=0).cuda()
                label = torch.cat(data[1],dim=0).cuda()
            else:
