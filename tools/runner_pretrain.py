...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses_box = AverageMeter(['Loss_box'])
        losses_rec = AverageMeter(['Loss_rec'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss,loss_box,loss_rec = base_model(points)
            try:
                loss.backward()
                # print("Using one GPU")
            except:
            ...
            if args.distributed:
                ...
                losses_box.update([loss_box.item()*1000])
                losses_rec.update([loss_rec.item()*1000])
            else:
                losses.update([loss.item()*1000])
                losses_box.update([loss_box*1000])
                losses_rec.update([loss_rec*1000])
            ...
            if idx % 20 == 0:
                print_log('[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) Losses = %s Losses_box = %s Losses_rec = %s lr = %.6f' %
                            (epoch, config.max_epoch, idx + 1, n_batches, batch_time.val(), data_time.val(),
                            ['%.4f' % l for l in losses.val()],['%.4f' % l for l in losses_box.val()],
                            ['%.4f' % l for l in losses_rec.val()], optimizer.param_groups[0]['lr']), logger = logger)
