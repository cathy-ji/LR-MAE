...
def main(args):
    ...
    if args.seed is not None:
        ...
        misc.set_random_seed(args.seed) # seed + rank, for augmentation
    ...
    classifier = pt.get_model(num_part).cuda()
    criterion = pt.get_loss().cuda()
...
## we use adamw and cosine scheduler
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            points[:, :, 0:3] = provider.rotate_perturbation_point_cloud(points[:,:,0:3])#seed5-15 seed0-4 
            #points[:, :, 0:3] = provider.jitter_point_cloud(points[:,:,0:3])#seed0-4
